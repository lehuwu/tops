...
var RoundsSql = {
	...
	summedRound: 'SELECT SUM(r.fee)::bigint AS "fees", SUM(r.tax)::BIGINT AS tax, ARRAY_AGG(r.reward) AS rewards, ARRAY_AGG(r.pk) AS delegates FROM (SELECT b."totalFee" AS fee, b.tax, b.reward, ENCODE(b."generatorPublicKey", \'hex\') AS pk FROM blocks b WHERE CEIL(b.height / ${activeDelegates}::float)::int = ${round} ORDER BY b.height ASC) r;',
	
	getTaxman: 'SELECT ENCODE(b."taxmanPublicKey", \'hex\') AS "taxmanPublicKey" FROM blocks b WHERE b.height=(${round} * ${activeDelegates}::int)',
