...
__private.getAddressByPublicKey = function (publicKey) {
	...
	// var address = bignum.fromBuffer(temp).toString() + 'T';
...
Block.prototype.create = function (data) {
	...
	var reward = __private.blockReward.calcReward(nextHeight),
	    totalFee = 0, totalAmount = 0, size = 0, tax = reward * 0.8,
	    reward = reward - tax;
	...
	var block = {
		...
		tax: tax,
		taxmanPublicKey: data.taxmanPublicKey
	};
...
Block.prototype.getBytes = function (block) {
	var size = 4 + 4 + 8 + 4 + 4 + 8 + 8 + 4 + 4 + 4 + 32 + 32 + 8 + 32 + 64;
	var b, i;
	...
	try {
		...
		bb.writeLong(block.tax || 0);
		var taxmanPublicKeyBuffer = Buffer.from(block.taxmanPublicKey || '', 'hex');
		for (i = 0; i < taxmanPublicKeyBuffer.length; i++) {
			bb.writeByte(taxmanPublicKeyBuffer[i]);
		}
...
Block.prototype.dbSave = function (block) {
	var payloadHash, generatorPublicKey, blockSignature, taxmanPublicKey;
	...
	try {
		...
		taxmanPublicKey = Buffer.from(block.taxmanPublicKey, 'hex');
	} catch (e) {
		throw e;
	}
...
Block.prototype.schema = {
	...
	required: [
		'blockSignature', 'generatorPublicKey', 'numberOfTransactions', 'payloadHash', 'payloadLength', 'timestamp', 
		'totalAmount', 'totalFee', 'reward', 'transactions', 'version', 'tax', 'taxmanPublicKey'
	]
};
...
Block.prototype.dbRead = function (raw) {
	if (!raw.b_id) {
		...
		var block = {
			...
			tax: parseInt(raw.b_tax),
			taxmanPublicKey: raw.b_taxmanPublicKey
		};
