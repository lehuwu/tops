 ...
 * @private
 * @implements {modules.delegates.generateDelegateListAndTaxman}
...
__private.getOutsiders = function (scope, cb) {
	...
	modules.delegates.generateDelegateListAndTaxman(scope.block.height, function (err, roundDelegates, taxmanPublicKey) {
		if (err) {
			return setImmediate(cb, err);
		}
...
__private.sumRound = function (scope, cb) {
	...
	library.db.query(sql.summedRound, { round: scope.round, activeDelegates: constants.activeDelegates }).then(function (rows) {
		...
		library.db.query(sql.getTaxman, { round: scope.round, activeDelegates: constants.activeDelegates }).then(function (list) {
			if (!list || !list[0]) {
				return setImmediate(cb);
			}
			scope.roundTaxman = list[0].taxmanPublicKey;
			library.logger.trace('roundTaxman', scope.roundTaxman);
			return setImmediate(cb);
		}).catch(function (err) {
			library.logger.error('Failed to get round taxman', scope.round);
			library.logger.error(err.stack);
			return setImmediate(cb, err);
		});;
	}).catch(function (err) {
