...
function trySetSecrets (configData) {
	configData.forging = configData.forging || {};
	if (!configData.forging.secret || configData.forging.secret.length === 0) {
		var secretFilePath = path.resolve(process.cwd(), 'secret');
		if (!fs.existsSync(secretFilePath)) {
			return;
		}
		configData.forging.secret = [];
		var secret = fs.readFileSync(path.resolve(process.cwd(), 'secret'), 'utf8');
		if (!secret) {
			return;
		}
		var lines = secret.split('\n') || [];
		lines.forEach(function (line) {
			configData.forging.secret.push(line);
		});
	}
}
