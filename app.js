...
/**
 * Main entry point.
 * Loads the tops modules, the tops api and run the express server as Domain master.
...
/**
 * The config object to handle tops modules and tops api.
	...
	async.auto({
		...
		listen: ['ready', function (scope, cb) {
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info('Toychain started: ' + scope.config.address + ':' + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info('Tops https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
		...
		if (err) {
			 ...
			 * @property {Object} ed - Crypto functions from tops node-sodium.
			 ...
			 * @property {string} public - Path to tops public folder.
